...
import { DiskNodeStorage } from './DiskNodeStorage';
...
describe('DiskNodeStorage.ts, DiskNodeStorage', () => {
  let dirPath: string;
  let storage: DiskNodeStorage;

  beforeEach(async () => {
    dirPath = await fs.mkdtemp(path.join(os.tmpdir(), 'raft-disk-node-'));
    storage = new DiskNodeStorage(dirPath);
  });
  ...
  it('should create all underlying disk storages', () => {
    expect(storage.meta).toBeInstanceOf(DiskMetaStorage);
    expect(storage.config).toBeInstanceOf(DiskConfigStorage);
    expect(storage.log).toBeInstanceOf(DiskLogStorage);
    expect(storage.snapshot).toBeInstanceOf(DiskSnapshotStorage);
  });
