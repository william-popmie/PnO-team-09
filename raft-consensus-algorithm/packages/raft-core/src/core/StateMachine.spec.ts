...
describe('StateMachine.ts, StateMachine', () => {
  ...
  it('should skip self vote when second voter check is false during candidate transition', async () => {
    ...
    configManager.isVoter.mockReturnValueOnce(true).mockReturnValueOnce(false);
  ...
  it('should log error when initial no-op append fails', async () => {
    ...
    expect(logger.error).toHaveBeenCalledWith(
      expect.stringContaining('failed to append initial no-op entry as Leader: append no-op failed'),
    );
  });
  ...
  it('should stringify non-Error values when initial no-op append fails', async () => {
    ...
    expect(logger.error).toHaveBeenCalledWith(
      expect.stringContaining('failed to append initial no-op entry as Leader: plain append no-op failure'),
    );
  });
  ...
  it('should append entries when log matches', async () => {
    const entries = [
      { index: 1, term: 1, type: LogEntryType.COMMAND, command: { type: 'set', payload: { key: 'x', value: 10 } } },
    ];
  ...
  it('should update commit index when leaderCommit is higher than current commit index', async () => {
    ...
    const response = await stateMachine.handleAppendEntries('node2', {
      ...
      prevLogIndex: 2,
      entries: [
        { index: 3, term: 1, type: LogEntryType.COMMAND, command: { type: 'set', payload: { key: 'x', value: 10 } } },
      ],
      leaderCommit: 3,
    });
  ...
  it('should clamp new commit index to last new entry index', async () => {
    ...
    await stateMachine.handleAppendEntries('node2', {
      ...
      prevLogIndex: 1,
      entries: [
        { index: 2, term: 1, type: LogEntryType.COMMAND, command: { type: 'set', payload: { key: 'x', value: 10 } } },
      ],
      leaderCommit: 10,
    });
  ...
  it('should return correct matchindex on succes', async () => {
    const response = await stateMachine.handleAppendEntries('node2', {
      ...
      prevLogIndex: 3,
      entries: [
        { index: 4, term: 1, type: LogEntryType.COMMAND, command: { type: 'set', payload: { key: 'x', value: 10 } } },
      ],
    });
  ...
  it('should ignore response with mismatched term', async () => {
    persistentState.getCurrentTerm.mockReturnValueOnce(1).mockReturnValueOnce(2).mockReturnValue(2);
    rpcHandler.sendRequestVote.mockResolvedValue({ term: 1, voteGranted: true });
    await stateMachine.becomeCandidate();
    await vi.waitFor(() => {
  ...
  it('should log error and return early from sendSnapshot when no snapshot is available', async () => {
    ...
    await vi.waitFor(() => {
      expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('has no snapshot to send'));
    });
  ...
  it('should update matchIndex when InstallSnapshot succeeds', async () => {
    ...
    await vi.waitFor(() => {
      expect(logger.info).toHaveBeenCalledWith(expect.stringContaining('successfully sent snapshot'));
    });
  });
  ...
  it('should log debug and return early from sendSnapshot response handler when leaderState is null', async () => {
    ...
    await vi.waitFor(() => {
      expect(logger.debug).toHaveBeenCalledWith(expect.stringContaining('received InstallSnapshotResponse'));
    });
  });
  ...
  it('should log Error instance when sendInstallSnapshot RPC throws', async () => {
    ...
    (rpcHandler as any).sendInstallSnapshot = vi.fn().mockRejectedValue(new Error('snapshot RPC failed'));

    await stateMachine.becomeLeader();

    await vi.waitFor(() => {
      expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('snapshot RPC failed'));
    });
  });
  ...
  it('should log plain string error when sendInstallSnapshot RPC throws non-Error', async () => {
    ...
    (rpcHandler as any).sendInstallSnapshot = vi.fn().mockRejectedValue('plain string snapshot error');

    await stateMachine.becomeLeader();

    await vi.waitFor(() => {
      expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('plain string snapshot error'));
    });
  });
  ...
  it('should not decrement nextIndex in catch block when leaderState is null at catch time', async () => {
    ...
    await vi.waitFor(() => {
      expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('rpc error'));
    });
