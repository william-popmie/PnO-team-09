...
describe('Storage.ts, StorageNumberUtil', () => {
  ...
  it('should throw for non-safe integer in assertSafeInteger', () => {
    expect(() => StorageNumberUtil.assertSafeInteger(Number.MAX_SAFE_INTEGER + 1, 'testField')).toThrow(
      'testField must be a safe integer',
    );
  });
  ...
  it('should throw when bigint is outside safe range in bigIntToSafeNumber', () => {
    ...
    expect(() => StorageNumberUtil.bigIntToSafeNumber(outOfRange, 'bigField')).toThrow(
      'bigField is outside JS safe integer range',
    );
  });
});
...
describe('Storage.ts, StorageCodec', () => {
  ...
  it('should throw when decoded number is not a safe integer', () => {
    ...
    expect(() => StorageCodec.decodeNumber(invalidBuffer)).toThrow(
      'Decoded number 9007199254740992 is not a safe integer',
    );
  });
