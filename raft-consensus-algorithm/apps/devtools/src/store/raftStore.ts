...
export const setStoreWebSocket = (ws: WebSocket | null) => {
  wsRef.current = ws;
};
...
export const useRaftStore = create<RaftStore>((set, get) => ({
  ...
  setNodeIds: (ids, config) => {
    ...
    for (const id of ids) {
      ...
      nodes[id] = makeNode(
        id,
        configToApply.learners.some((m) => m.id === id),
        member?.address ?? '',
      );
    }
  ...
  pushEvent: (event) =>
    set((state) => ({ events: [event, ...state.events].slice(0, 100), totalEventCount: state.totalEventCount + 1 })),
  processEvent: (event) => {
    switch (event.type) {
      ...
      case 'MessageSent': {
        ...
        const isHeartbeat: boolean =
          event.messageType === 'AppendEntries' && (event.payload as { entries: unknown[] })?.entries?.length === 0;
        const isPreVote: boolean =
          event.messageType === 'RequestVote' && Boolean((event.payload as { preVote?: boolean })?.preVote);
      ...
      case 'NodeCrashed': {
        set((state) => {
          ...
          return {
            ...
            arrows: state.arrows.filter((a) => a.fromNodeId !== event.nodeId && a.toNodeId !== event.nodeId),
          };
        });
        break;
      }
      ...
      case 'LinkCut': {
        set((state) => ({
          cutLinks: new Set([...state.cutLinks, `${event.nodeA}-${event.nodeB}`, `${event.nodeB}-${event.nodeA}`]),
        }));
        break;
      }
        ...
        set((state) => ({
          nodes: {
            ...
            [event.nodeId]: {
              ...
              logEntries: state.nodes[event.nodeId]?.logEntries.filter((e) => e.index > event.lastIncludedIndex) ?? [],
            },
          },
  ...
  toggleMessageVisibility: (messageType) =>
    set((state) => ({
      messageVisibility: {
  ...
  reset: () =>
    set({
      ...
      snapshottingNodes: new Set(),
      installingSnapshotNodes: new Set(),
      clusterConfig: defaultConfig,
      pendingConfigChange: false,
    }),
}));
